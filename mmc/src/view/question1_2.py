...
mDistance = []
lookDis = [301338, 369199, 207489, 135677, 240308, 260152, 249970, 264196, 168703, 178620, 138703, 255623, 267707, 379216, 186912, 175397, 228214, 150557, 219145, 193743,0]
for i in range(0,21):
    tRow = []
    for j in range(0,21):
        if i!=j:
            tRow.append(int(math.sqrt((centers[j,0]-centers[i,0])**2+(centers[j,1]-centers[i,1])**2)*38.2))
        else:
...
def findNextPath(mcurrentPath, pathLeft, distanceLeft, mareaLeft):
    ...
    for item in pathLeft:
        ...
        if row1[item] + row2[20]+ areaLeft[item] <= distanceLeft:
            # 将该路径保存
        ...
        elif row1[item] + row2[20] < distanceLeft:
            scanDis = distanceLeft - row1[item] - row2[20]
            _currentPath = currentPath.copy()
            _currentPath.append((item, scanDis))
            # areaLeft[item] -= scanDis
            _currentPath.append((20, 0))
            currentScan = 0
